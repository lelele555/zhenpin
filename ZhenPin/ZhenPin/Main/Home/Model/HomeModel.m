...
-(instancetype)initWithDictionary:(NSDictionary *)dict error:(NSError *__autoreleasing *)err{
    ...
    if(self){
        ...
        _brandArr=[BrandModel arrayOfModelsFromDictionaries:arr error:nil];
    }
    return self;
}
    ...
    [BaseRequest getWithUrl:url para:nil call:^(NSData *data, NSError *error){
        if(error==nil){
            ...
            if([[NSString stringWithFormat:@"%@",arr[0][@"groupId"]] isEqualToString:@"1"]){
                ...
                NSMutableArray *caArr=[[NSMutableArray alloc]init];
                for(int i=0;i<cArr.count;i++){
                    [caArr addObject:cArr[i][@"name"]];
                }
                NSArray *contentArr=[ItemTModel arrayOfModelsFromDictionaries:arr[2][@"elementResult"] error:nil];
...
-(void)setValue:(id)value forKey:(NSString *)key{
    ...
    if([key isEqualToString:@"products"]){
        ...
        _brandArr=[BrandModel arrayOfModelsFromDictionaries:value error:nil];
