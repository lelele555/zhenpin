...
-(instancetype)initWithFrame:(CGRect)frame style:(MyScrollViewStyle)style imgArray:(NSArray*)imgArray titleArr:(NSArray*)titleArr timer:(NSTimeInterval)time{
    ...
    if (self){
        ...
        if (titleArr==nil||[titleArr count]==0){
            sv=[SSTGDScrollView scrollViewWithFrame:frame imgArr:imgArr titleArr:nil time:time imgPick:isImgPick];
        }else{
            style = MyScrollViewStyleDownLabel;
            sv=[SSTGDScrollView scrollViewWithFrame:frame imgArr:imgArr titleArr:titleArr time:time imgPick:isImgPick];
        }
        ...
        if (style == MyScrollViewStylePageC){
            ...
            pageView.backgroundColor=[UIColor colorWithWhite:0.90 alpha:0.05];
